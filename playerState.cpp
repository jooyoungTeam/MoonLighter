...
void playerIdleState::update(player & player)
{
	...
	//플레이어 텔레포트
	if (SCENEMANAGER->getCurrentScene() != "마을씬" && SCENEMANAGER->getCurrentScene() != "샵씬" && INVENTORY->getGold() >= 200)
	{
...
void playerDieState::update(player & player)
{
	...
	SOUNDMANAGER->stop("총알쏘기");
	SOUNDMANAGER->stop("총알장전");
	SOUNDMANAGER->stop("총알터짐");
	SOUNDMANAGER->stop("총알어텍");
	SOUNDMANAGER->stop("레드슬라임공격");
	SOUNDMANAGER->stop("슬라임펑");
	SOUNDMANAGER->stop("골렘공격");
	SOUNDMANAGER->stop("끌어당김");
	SOUNDMANAGER->stop("돌떨어짐");
	SOUNDMANAGER->stop("돌떨어짐2");
	SOUNDMANAGER->stop("손떨어짐");
	SOUNDMANAGER->stop("손빧음");
	SOUNDMANAGER->stop("폭발");
	SOUNDMANAGER->stop("돌사라짐");
